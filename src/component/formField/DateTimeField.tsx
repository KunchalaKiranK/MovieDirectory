...
const DateTimeField = (props: TYPE.QuestionProps) => {
  const {
    Name,
    StationName,
    QuestionGroupName,
  ...
  const handleChange = (d: Date) => {
    setUserDate(saveDateTimeFormat(d));

    // validation and save the response
    let valid = handleValidate(props, [saveDateTimeFormat(d)]);
    if (valid === 'true') {
      setError('');
      if ((userResponse ? userResponse[0] : null) !== saveDateTimeFormat(d)) {
        let userResponses = {
          scriptType: 'survey',
          QuestionName: Name,
          val: [saveDateTimeFormat(d)],
          StationName,
          QuestionGroupName,
        };
